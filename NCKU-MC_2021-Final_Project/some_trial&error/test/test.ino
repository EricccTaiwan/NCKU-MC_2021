 ...
 int  n=0;
 int  k=0;
 int  cm_f,duration_b, cm_b,duration_f; 
 int  first=0;
 int  cross=0;
 void setup() {
 Serial.begin(9600);
...
void loop() {
  ...
  cm_f=ultrasonic.Ranging(CM);
    if(cm_f<20 and cm_f>0){//第一關進去
    ...
    else if(side_r<black and middle_r>black and middle_l>black and side_l>black and (time/1000)>60){
      while(1){
      go_stop();//岔路進
      delay(10);
      analogWrite(11,80);
      ...
      cross==8;
        ...
        else if(side_r>black and ( middle_r>black or middle_l>black) and side_l<black and cross==8){
      while(1){
      go_stop();//岔路out
      delay(10);
      analogWrite(11,85);
    ...
    else if(cm_f<55 and cm_f>40 and middle_l<black and middle_r<black and side_l<black and side_r<black and (time/1000)>100){ //倒車入庫
      while(1){//自轉
          go_stop();
          delay(50);
          int middle_r=analogRead(A3);
