...
void loop() {
  ...
  //cm_f=ultrasonic.Ranging(CM);
  if ((time/1000)>110){
    front();
  }

  if((time/1000)>25 and first_time==0 and (time/1000)<35){
    front();
  }
  if(cm_f<20 and cm_f>0){//第一關進去
      while(1){
          go_stop();
          delay(100);
          int middle_r=analogRead(A3);
   ...
   else if(side_r>black and side_l>black and (time/1000)>60){
      go_right();//中右黑
      delay(wait_lilturn);
      go_stop();
      delay(wait_stop);
    }
  else if(side_r<black and middle_r>black and middle_l>black and side_l>black and (time/1000)>60 and (time/1000)<85 and cross==0){
      while(1){
      go_stop();//岔路進
      delay(10);
      analogWrite(11,83);
    ...
    else if(side_r>black and ( middle_r>black or middle_l>black)and (time/1000)>75 and (time/1000)<85 and cross==8 ){
      while(1){
      go_stop();//岔路out
      delay(10);
      analogWrite(11,100);
    ...
    else if(cm_f<50 and cm_f>45 and middle_l<black and middle_r<black and side_l<black and side_r<black and (time/1000)>110 ){ //倒車入庫
      while(1){//自轉
          go_stop();
          delay(50);
          int middle_r=analogRead(A3);
    ...
    else if (cm_b>0 and cm_b<25  and n==4){ //停車
      go_stop();
      delay(5000);
      exit(0); 
    }
